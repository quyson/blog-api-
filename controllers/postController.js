...
const onePost = async (req, res) => {
  try {
    ...
    const resultPost = results[0];
    const resultComments = results[1];
    res.send({
      success: true,
...
const likePost = async (req, res) => {
  const result = await Post.findById(req.body.id, "likes");
  const updateLikes = result.likes + 1;
  Post.findByIdAndUpdate(result.id, { likes: updateLikes }, { new: true })
    .then((result) => {
      res.status(200).send({
        success: true,
